...
const Create = () => {
    ...
    const GetData = () => {
        ...
        Promise.all([
        ...
        ]).then(([countryRes, leagueRes, characteristicRes]) => {
            console.log('Characteristic data:', characteristicRes.data)
            setCountry(countryRes.data || [])
            setLeague(leagueRes.data || [])
            setCharacteristic(
                Array.isArray(characteristicRes.data) 
            ...
            setMessage(
                <MyMessage
                    messageText="Error loading data. Please refresh the page."
                    messagecolor="red"
                />
            )
        })
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            const submitData = {
                ...
                characteristic: Array.isArray(values.characteristic) 
                    ? values.characteristic.map(Number)
                    : [Number(values.characteristic)]
            }

            console.log('Submitting data:', submitData)

            AxiosInstance.post(`footballclub/`, submitData)
            .then((response) => {
            ...
            .catch((error) => {
                ...
                setMessage(
                    <MyMessage
                        messageText={error.response?.data?.message || "Error submitting data. Please check all fields."}
                        messagecolor={"red"}
                    /> 
                )
            })
                            ...
                            <Box sx={{marginTop:'30px'}}>
                                <MultiSelectForm
                                    ...
                                    onChange={(e) => {
                                        ...
                                        formik.setFieldValue('characteristic', Array.isArray(selectedValues) ? selectedValues : [])
                                    }}
                                    onBlur={formik.handleBlur}
